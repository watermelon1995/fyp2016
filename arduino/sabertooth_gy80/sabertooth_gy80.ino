...
unsigned long lastMilli_acc;
unsigned long interval_acc;
...
void read_acc() {
  //--------------X
  int i;
  unsigned long this_milli = millis();
  X_out=0;
  for(i=0;i<10;i++) {
  ...
  interval_acc = this_milli - lastMilli_acc;
  pose_x=accRaw[0]*interval_acc;
  pose_y=accRaw[1]*interval_acc;
  lastMilli_acc = this_milli;
}
